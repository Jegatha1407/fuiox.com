...
#ab-canvas { position:relative;  height:1800px; }
...
@push('topbar_actions')
<button onclick="abOpenAiModal()" style="padding:9px 14px;background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;border:none;border-radius:9px;font-size:12.5px;font-weight:700;cursor:pointer;font-family:inherit;margin-right:8px;"><i class="bi bi-stars me-1"></i> AI Generate</button>
<span id="abStatus" style="font-size:12px;color:#888;margin-right:12px;"></span>
<button onclick="abSave()" style="padding:9px 18px;background:#25d366;color:#fff;border:none;border-radius:9px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;"><i class="bi bi-floppy-fill me-1"></i> Save Flow</button>
@endpush

@section('content')

<div class="ab-wrap" style="height:calc(100vh - 110px);">
...
<div id="abAiModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:99999;align-items:center;justify-content:center;"><div style="background:#161d26;border-radius:18px;padding:28px;width:500px;max-width:92vw;border:1px solid rgba(255,255,255,0.1);"><div style="font-size:18px;font-weight:800;color:#fff;margin-bottom:6px;"><i class=\"bi bi-stars me-2\" style=\"color:#667eea;\"></i>AI Flow Generator</div><div style="font-size:12.5px;color:rgba(255,255,255,0.4);margin-bottom:18px;">Describe your flow and AI will build it instantly</div><textarea id="abAiPrompt" rows="4" placeholder="e.g. Build a hospital appointment flow with Cardiology, Neurology and General departments." style="width:100%;padding:12px;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.12);border-radius:10px;color:#fff;font-size:13px;outline:none;font-family:inherit;resize:vertical;box-sizing:border-box;"></textarea><div id="abAiError" style="display:none;background:rgba(229,57,53,0.15);color:#ef5350;padding:10px 14px;border-radius:8px;font-size:12.5px;margin-top:10px;"></div><div style="display:flex;gap:10px;margin-top:16px;"><button onclick="document.getElementById('abAiModal').style.display='none';" style="flex:1;padding:11px;border:none;border-radius:9px;background:rgba(255,255,255,0.07);color:#fff;cursor:pointer;font-family:inherit;">Cancel</button><button id="abAiGenerateBtn" onclick="abGenerateFlow()" style="flex:1;padding:11px;border:none;border-radius:9px;background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;font-weight:700;cursor:pointer;font-family:inherit;"><i class="bi bi-stars me-1"></i> Generate Flow</button></div></div></div>
@endsection
...
async function abGenerateFlow(){
    ...
    try{
        ...
        if(!d.success){errDiv.textContent=d.error||"Failed";errDiv.style.display="block";return;}
        abNodes=[];abConns=[];
        ...
        abToast("Flow generated! Review and save.");
    }catch(err){

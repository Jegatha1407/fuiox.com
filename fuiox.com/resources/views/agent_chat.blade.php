                ...
                <div class="chat-top-menu-wrap" id="chatTopMenuWrap">
                    ...
                    <div class="chat-top-menu" id="chatTopMenu">
                        ...
                        <div class="ai-toggle-row">
                            <span><i class="bi bi-robot me-2"></i>AI Reply</span>
                            <label class="ai-switch">
                                <input type="checkbox" id="aiToggle" {{ $user->bot_status === 'on' ? 'checked' : '' }} onchange="chatToggleAI(this)">
            ...
            <div class="emoji-wrap">
                ...
                <div class="emoji-panel" id="chatEmojiPanel">
                    ...
                    <div class="emoji-search-wrap">
                        <input type="text" placeholder="Search emoji…" oninput="chatSearchEmoji(this.value)">
                    </div>
...
function chatLoadUsers(){
    fetch('/chat/users').then(r=>r.json()).then(data=>{ chatAllUsers=data; chatRenderUsers(data); }).catch(()=>{});
}
...
function chatBuildContent(m){
    ...
    switch(m.media_type){
        ...
        case 'video': return `<video controls style="max-width:260px;max-height:180px;border-radius:6px;display:block;"><source src="${url}"></video>${m.media_caption?`<div class="msg-text" style="padding-right:0;">${chatEsc(m.media_caption)}</div>`:''}`;
...
function chatToggleAI(cb){ fetch('/chat/toggle',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}'},body:JSON.stringify({status:cb.checked?'on':'off'})}); }
